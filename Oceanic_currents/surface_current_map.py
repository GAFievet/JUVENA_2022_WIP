...
def plot_oceanic_currents(u, v, lons, lats, date):
	...
	plt.title(f'Surface Oceanic Current Velocities on {date.strftime("%Y-%m-%d")}, (72h low-passed and daily '
	          f'averaged)')
...
if __name__ == '__main__':
	...
	try:
		with (open(path2file, 'rb') as f):
			...
			u_all, v_all, lons, lats, t = data2plot['u_d_avg'], data2plot['v_d_avg'], data2plot['lon_ibi'], data2plot[
				'lat_ibi'], data2plot['t2']
			u, v = u_all[:, :, 0], v_all[:, :, 0]

			# Plot
			plot_oceanic_currents(u, v, lats, lons, t[0])
